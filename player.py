...
class Player(pygame.sprite.Sprite):
    ...
    def animate(self):
        ...
        # set the rect to the image size
        # when on the ground
    ...
    def run_dust_animation(self):
        if self.status == 'run' and self.on_ground:
            ...
            dust_particle = self.dust_run_particles[int(
                self.dust_frame_index) % len(self.dust_run_particles)]
    ...
    def get_input(self):
        ...
        elif keys and (keys[pygame.K_UP] or keys[pygame.K_w] or keys[pygame.K_SPACE]) and self.on_ground:
            self.jump()
    ...
    def get_status(self):
        if self.direction.y < 0:
            self.status = 'jump'
        elif self.direction.y > 1:  # 1 or any number greater than self.gravity -> 0.8
            self.status = 'fall'
        else:
